...
Observing 5 fields for ten years up to \zcomp $\sim$ 0.9 would certainly give access to a large sample of well-measured \sne~(around 11k) but also to an unrealistic scenario (DD budget: 87\%). The only way to reach higher \zcomp~while maintaining a reasonable budget is to reduce the number of seasons of observation. This is the purpose of the Rolling Deep Drilling (RDD) strategy characterized by a limited number of seasons of observation per field (typically 2) and a large number of visits per observing night (more than 200 for higher \zcomp).

\par
Building a consistent RDD strategy is not only a matter of adjusting LSST survey settings to stay within a reasonable budgetary envelope. Spectral measurements will be essential to the success of the supernovae program, to ensure a spectroscopic follow-up of all the well-measured supernovae collected (photometric classification), and to estimate host redshifts with high accuracy. Among available spectroscopic resources of interest for the RDD survey are the Primary Focus Spectrograph (PFS), and 

have to be taken into account to design an optimal LSST-RDD survey for supernovae. 

\begin{figure}[htbp]
\begin{center}
  \includegraphics[width=0.9\textwidth]{budget_zcomp.png}
  \caption{DD budget as a function of the redshift of completeness for a set of scenarios where the redshift of completeness of \cosmos~and \xmm~is set to 0.9. Subscripts correspond to the number of seasons of observation and superscripts to the redshift limit. ER stands for Euclid/Roman field. Highest \zcomp~values are reached for a minimal strategy composed of two seasons of observations of \cosmos~and \xmm~with a depth of 0.9 plus 4 seasons of observation of the \adfs~ field (1 pointing). Coloured areas correspond to a variation of the number of $y$-band visits (20$\leq N_{visits}^y \leq$ 80).}\label{fig:budget_zcomp}
\end{center}
\end{figure}
\par

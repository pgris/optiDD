...
Four sets of observing strategies may be extracted from Tab. \ref{tab:os} according to the filter allocation per night, which is the parameter that has the most significant impact on the redshift limit value : \osfamily{baseline-like}~ (10 observing strategies), with two sets of filter distributions ($g/r/i/z/y$ and $u/g/r/i/y$, with $(N_u,N_g,N_r,N_i,N_z,N_y)=(8-16,10,20,20,26,20)$), \osfamily{agnddf} ($g/r/i/z/y$ with $(N_g,N_r,N_i,N_z,N_y)=(1,1,3,5,4)$), \osfamily{daily} ($g/r/i/z/y$ with $(N_g,N_r,N_i,N_z,N_y)=(1,1,2,2,2)$), \osfamily{descddf} (($g/r/i$ interleaved with $z/y$ with $(N_g,N_r,N_i,N_z,N_y)=(2,4,8,25,4)$). Estimation of the pair metric (\nsn,\zcompb) (defined in \ref{sec:metrics}) for these families shows (Fig. \ref{fig:nsn_zlim_zoom}) that higher redshift limits are reached for \osfamily{baseline-like} family. Most (9/10) of these observing strategies reach \zlimcopb$\sim$0.65. ddf\_heavy, the one with the largest budget (13.4\%),  reaches \zlimcompb$\sim$0.72 and also collects the larger number of well-sample \sne~($\sim$ 6000).

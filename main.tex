...
\par
In SALT2, model uncertainties of $g$ and $r$-band (rest-frame UV) light-curves fluxes are large(\citealt{Guy_2007}). $g$ and $r$ observations with relative error model larger than 10$\%$ have not been considered in this study. This requirement implies that the list of filters useful to measure photometric light-curves (observer-frame) is redshift-dependent: $gri$ bands up to $z\lesssim$0.3-0.4, $rizy$ bands for $0.4\lesssim z \lesssim 0.6-0.7$, and $ izy$ bands for $z\gtrsim 0.7$.
\par
...
Observing 5 fields for ten years up to \zcomp $\sim$ 0.9 would certainly give access to a large sample of well-measured \sne~(around 11k) but also to an unrealistic scenario (DD budget: 87\%). The only way to reach higher \zcomp~while maintaining a reasonable budget is to reduce the number of seasons of observation. This is the purpose of the Rolling Deep Drilling (RDD) strategy characterized by a limited number of seasons of observation per field (typically 2) and a large number of visits per observing night.

\par
Building a consistent RDD strategy is not only a matter of adjusting LSST survey settings to stay within a reasonable budgetary envelope. Photometry will be used to collect the sample of well-measured \sne and to estimate their host redshifts. But spectroscopic measurements will be necessary, either to confirm the type of the supernovae collected (and used for photometric classification), and to estimate host redshifts with high accuracy. Available spectroscopic resources have to be taken into account to design an optimal RDD survey for supernovae.
